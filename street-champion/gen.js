...
const CSS=`
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  #base,#shop,#select,#slots{display:none;flex-direction:column;background:linear-gradient(#1a1226,#120f20);padding:10px;}
  #base h2,#shop h2,#select h2,#slots h2{color:var(--gold);font-size:15px;text-align:center;margin-bottom:6px;}
  ...
  #select,#slots{gap:10px;padding:14px 10px;}
  #slots .card{margin-bottom:0;}
  #slots .row b{color:var(--gold);}
...
const BODY=`
  <div id="title">
    ...
    <div style="display:flex;flex-direction:column;gap:10px;align-items:center;">
      <button class="btn blink" id="startBtn">▶ はじめる</button>
    </div>
    <div class="sub" style="font-size:10px;" id="saveNote">3つの スロットに セーブできます。</div>
  </div>

  <div id="slots">
    <h2>セーブスロット</h2>
    <div class="chars" id="slotList"></div>
    <button class="btn sm" id="slotBack" style="align-self:center;">← もどる</button>
  </div>
...
(function(){
  ...
  var CHARS={
    jin:{name:'ジン',spr:'jin',move:'きこうけん',
      desc:'ながれの からてか。HP・こうげき・き すべて へいきん。スタート:ドリンク+2',
      kit:function(){P.herb+=2;}},
    dandy:{name:'ダンディ',spr:'dandy',move:'コークスクリュー',
      desc:'じゅうりょうきゅうボクサー。HPと こうげきが たかい。きは すくなめ。スタート:レザーグローブ',
      kit:function(){P.weapons.push('leather');P.equipped='leather';}},
    neo:{name:'ネオ',spr:'neo',move:'サイコパンチ',
      desc:'サイコパワーの しょうねん。きが おおく ひっさつ れんぱつ。HPは ひくめ。スタート:はどうの まき',
      kit:function(){P.rings.push('hadou');P.ring='hadou';}}
  };
  var CHARORDER=['jin','dandy','neo'];
  // キャラごとの せいのうさ（flat ＋ レベルごとの のび）
  var CHARMOD={
    jin:{hp:0,hpg:0,mp:0,mpg:0,atk:0,atkg:0},
    dandy:{hp:8,hpg:2,mp:-2,mpg:-1,atk:2,atkg:1},
    neo:{hp:-4,hpg:-1,mp:6,mpg:2,atk:0,atkg:0}
  };
  function cmod(char){return CHARMOD[char]||CHARMOD.jin;}
  ...
  var P={};
  function maxHpFor(lv,char){var m=cmod(char||P.char);return 30+(lv-1)*8+m.hp+(lv-1)*m.hpg;}
  function maxMpFor(lv,char){var m=cmod(char||P.char);return 8+(lv-1)*3+m.mp+(lv-1)*m.mpg;}
  function atkFor(lv,char){var m=cmod(char||P.char);return 6+(lv-1)*2+m.atk+(lv-1)*m.atkg;}
  ...
  function saveGame(){if(!HAS_STORAGE)return;try{localStorage.setItem(slotKey(curSlot),JSON.stringify(P));}catch(e){}}
  function slotMeta(i){if(!HAS_STORAGE)return null;try{var s=localStorage.getItem(slotKey(i));return s?JSON.parse(s):null;}catch(e){return null;}}
  function normalize(d){
    ...
    d.hp=maxHpFor(d.lv,d.char);d.mp=maxMpFor(d.lv,d.char);return d;
  }
  function loadGame(i){
    curSlot=i;if(!HAS_STORAGE)return false;
    try{var s=localStorage.getItem(slotKey(i));if(!s)return false;P=normalize(JSON.parse(s));return true;}catch(e){return false;}
  }
  ...
  var elTitle=document.getElementById('title'),elBase=document.getElementById('base'),
      ...
      elSelect=document.getElementById('select'),elSlots=document.getElementById('slots');
  function show(w){elTitle.style.display=w==='title'?'flex':'none';
    ...
    elSlots.style.display=w==='slots'?'flex':'none';}
  ...
  function renderSlots(){
    var L=document.getElementById('slotList');L.innerHTML='';
    for(var i=0;i<SLOTS;i++){(function(i){
      var d=slotMeta(i),card=document.createElement('div');card.className='card';
      var info=document.createElement('div');info.className='row';
      var btns=document.createElement('div');btns.className='baseBtns';btns.style.marginTop='6px';
      if(d){
        var cn=(CHARS[d.char]&&CHARS[d.char].name)||'ファイター';
        info.innerHTML='<b>スロット'+(i+1)+'</b><span>'+cn+' / LV'+(d.lv||1)+' / せいは'+(d.clears||0)+'</span>';
        var cont=document.createElement('button');cont.className='btn sm';cont.textContent='▶ つづきから';
        cont.addEventListener('click',function(){sfx('select');if(loadGame(i)){renderBase();show('base');try{window.scrollTo(0,0);}catch(e){}}});
        var del=document.createElement('button');del.className='btn sm';del.style.borderColor='var(--hp)';del.style.color='var(--hp)';del.textContent='けす';
        var armed=false;
        del.addEventListener('click',function(){if(!armed){armed=true;del.textContent='ほんとに けす？';setTimeout(function(){armed=false;del.textContent='けす';},2500);return;}
          try{localStorage.removeItem(slotKey(i));}catch(e){}sfx('select');renderSlots();});
        btns.appendChild(cont);btns.appendChild(del);
      }else{
        info.innerHTML='<b>スロット'+(i+1)+'</b><span style="color:var(--dim)">あき</span>';
        var nw=document.createElement('button');nw.className='btn sm';nw.textContent='＋ はじめから';
        nw.addEventListener('click',function(){sfx('select');curSlot=i;startSelect();});
        btns.appendChild(nw);
      }
      card.appendChild(info);card.appendChild(btns);L.appendChild(card);
    })(i);}
  }
  function startSlots(){renderSlots();show('slots');try{window.scrollTo(0,0);}catch(e){}}

  document.getElementById('startBtn').addEventListener('click',startSlots);
  document.getElementById('slotBack').addEventListener('click',function(){show('title');});
  document.getElementById('selBack').addEventListener('click',startSlots);
